...
class TrainingJob(Base):
    ...
    starter = relationship(
        "User", back_populates="training_jobs", foreign_keys=[started_by]
    )
...
class TrainingDataSample(Base):
    ...
    dataset_id = Column(
        UUID(as_uuid=True),
        ForeignKey("training_datasets.id"),
        nullable=False,
        index=True,
    )
    ...
    )  # user_generated, synthetic, imported
    source_reference = Column(String(200), nullable=True)
    parent_sample_id = Column(
        UUID(as_uuid=True), ForeignKey("training_data_samples.id"), nullable=True
    )
