...
class ShareKey(Base):
    ...
    therapist_email = Column(
        String(255), nullable=False, index=True
    )  # Email for unregistered therapists

    # Permissions and access control
    permission_level = Column(
        String(20), nullable=False, default=SharePermission.READ_ONLY.value
    )
    ...
    )  # "patient_request", "expired", "security"
    ...
    )  # Only share data after this date
    ...
    )  # Is this therapist an emergency contact?
    crisis_access = Column(
        Boolean, default=False
    )  # Can access during crisis situations?
    ...
    patient = relationship(
        "User", foreign_keys=[patient_id], back_populates="patient_share_keys"
    )
    therapist = relationship(
        "User", foreign_keys=[therapist_id], back_populates="therapist_share_keys"
    )
    access_logs = relationship(
        "ShareKeyAccessLog", back_populates="share_key", cascade="all, delete-orphan"
    )
...
class ShareKeyAccessLog(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    share_key_id = Column(
        UUID(as_uuid=True), ForeignKey("share_keys.id"), nullable=False
    )

    # Access details
    accessed_at = Column(DateTime, server_default=func.now(), nullable=False)
    accessed_resource = Column(
        String(100), nullable=False
    )  # "mood_entries", "therapy_notes", etc.
    ...
    # Actions performed
    action_type = Column(
        String(50), nullable=False, default="view"
    )  # "view", "export", "comment"
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "user_agent": (
                self.user_agent[:100] if self.user_agent else None
...
class TherapistNoteAccess(Base):
    ...
    therapy_note_id = Column(
        UUID(as_uuid=True), ForeignKey("therapy_notes.id"), nullable=False
    )
    ...
    granted_by_patient = Column(
        Boolean, default=True, nullable=False
    )  # Patient explicitly granted access
...
class DataSharingAudit(Base):
    ...
    )  # "share_key_created", "data_accessed", "key_revoked"
    ...
    # Related objects
    share_key_id = Column(
        UUID(as_uuid=True), ForeignKey("share_keys.id"), nullable=True
    )
...
class PatientSharingPreferences(Base):
    ...
    allow_research_participation = Column(
        Boolean, default=False
    )  # For research studies

    # Emergency settings
    emergency_sharing_enabled = Column(Boolean, default=False)
    emergency_contact_therapist_id = Column(
        UUID(as_uuid=True), ForeignKey("users.id"), nullable=True
    )
    ...
    # Relationships
    patient = relationship("User", foreign_keys=[patient_id])
    emergency_contact_therapist = relationship(
        "User", foreign_keys=[emergency_contact_therapist_id]
    )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "emergency": {
                ...
                "contact_therapist_id": (
                    str(self.emergency_contact_therapist_id)
                    if self.emergency_contact_therapist_id
                    else None
                ),
...
# Access logs indexes
Index(
    "idx_access_logs_share_key_time",
    ShareKeyAccessLog.share_key_id,
    ShareKeyAccessLog.accessed_at,
)
Index(
    "idx_access_logs_resource_time",
    ShareKeyAccessLog.accessed_resource,
    ShareKeyAccessLog.accessed_at,
)

# Audit trail indexes
Index(
    "idx_audit_patient_event", DataSharingAudit.patient_id, DataSharingAudit.event_type
)
Index(
    "idx_audit_therapist_event",
    DataSharingAudit.therapist_id,
    DataSharingAudit.event_type,
)
