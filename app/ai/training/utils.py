...
def load_checkpoint(
    ...
    logger.info(
        f"✅ Checkpoint loaded (Epoch: {checkpoint['epoch']}, Step: {checkpoint['step']})"
    )
...
def cleanup_old_checkpoints(checkpoint_dir: str, keep_last_n: int = 3):
    ...
    checkpoints = sorted(
        checkpoint_dir.glob("checkpoint_epoch_*.pt"), key=lambda x: x.stat().st_mtime
    )
...
class EarlyStopping:
    ...
    def __init__(self, patience: int = 3, mode: str = "min", delta: float = 0.0):
        ...
        self.is_better = lambda score, best: (
            score < best - delta if mode == "min" else score > best + delta
        )
