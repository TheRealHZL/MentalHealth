...
class AIEngine:
    ...
    async def _load_tokenizer(self):
        ...
        if tokenizer_path.exists():
            ...
            self.tokenizer = MindBridgeTokenizer(vocab_size=self.settings.VOCAB_SIZE)
    ...
    async def _load_emotion_classifier(self):
        ...
        from app.ai.models.emotion_classifier import EmotionClassifier
    ...
    async def _load_mood_predictor(self):
        ...
        from app.ai.models.mood_predictor import MoodPredictor
    ...
    async def _load_chat_generator(self):
        ...
        from app.ai.models.chat_generator import ChatGenerator
    ...
    async def _load_sentiment_analyzer(self):
        ...
        from app.ai.models.sentiment_analyzer import SentimentAnalyzer
    ...
    async def predict_mood(
        ...
        try:
            ...
            token_sequences = [
                self.tokenizer.encode(t) for t in input_texts[-5:]
            ]  # Last 5 texts

            # Pad sequences
            max_len = max(len(seq) for seq in token_sequences)
            padded_sequences = [
                seq + [0] * (max_len - len(seq)) for seq in token_sequences
            ]
    ...
    def _update_stats(
        self, model_name: str, confidence: float, latency: float, length: int = 0
    ):
