...
from app.models.encrypted_models import EncryptedMoodEntry
from app.schemas.ai import (MoodEntryCreate, MoodEntryResponse,
                            MoodEntryUpdate, PaginatedResponse,
...
async def create_mood_entry(
    ...
    try:
        ...
        return {**mood_entry.__dict__, "ai_analysis": ai_analysis}
...
async def get_mood_entries(
    ...
    min_mood: Optional[int] = Query(
        None, ge=1, le=10, description="Minimum mood score"
    ),
    max_mood: Optional[int] = Query(
        None, ge=1, le=10, description="Maximum mood score"
    ),
    user_id: str = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_async_session),
) -> Dict[str, Any]:
...
async def update_mood_entry(
    ...
    try:
        ...
        updated_entry = await mood_service.update_mood_entry(
            entry_id=entry_id, user_id=user_id, update_data=update_data
        )
        ...
        return {**updated_entry.__dict__, "ai_analysis": ai_analysis}
...
async def delete_mood_entry(
    ...
    try:
        ...
        return {"success": True, "message": "Stimmungseintrag erfolgreich gelöscht"}
...
async def quick_mood_entry(
    mood_score: int = Query(
        ..., ge=1, le=10, description="Schnelle Stimmungsbewertung"
    ),
    ...
    try:
        ...
        quick_entry = await mood_service.create_quick_mood_entry(
            user_id=user_id, mood_score=mood_score, note=note
        )
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "insights": insights,
                "progress_summary": await mood_service.generate_progress_summary(
                    user_id
                ),
                "motivational_message": mood_service.get_motivational_message(
                    weekly_stats
                ),
            },
        }
...
async def get_encrypted_mood_entry(
    ...
    try:
        result = await db.execute(
            select(EncryptedMoodEntry).where(
                and_(
