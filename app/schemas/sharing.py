...
class ShareKeyCreate(BaseModel):
    ...
    permission_level: SharePermission = Field(
        SharePermission.READ_ONLY, description="Permission level"
    )
    ...
    max_sessions: Optional[int] = Field(
        None, ge=1, le=1000, description="Maximum sessions"
    )
    notes: Optional[str] = Field(
        None, max_length=1000, description="Notes about sharing"
    )

    # Advanced options
    date_range_start: Optional[datetime] = Field(
        None, description="Only share data after this date"
    )
    date_range_end: Optional[datetime] = Field(
        None, description="Only share data before this date"
    )
    exclude_tags: Optional[List[str]] = None
    emergency_contact: bool = Field(
        False, description="Is this therapist an emergency contact?"
    )
    crisis_access: bool = Field(
        False, description="Can access during crisis situations?"
    )
...
class TherapistAccessRequest(BaseModel):
    ...
    message: Optional[str] = Field(
        None, max_length=500, description="Message to patient"
    )
