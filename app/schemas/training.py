...
class TrainingDatasetCreate(BaseSchema):
    ...
    description: str = Field(
        ..., min_length=10, max_length=1000, description="Dataset description"
    )
...
class TrainingDataUpload(BaseSchema):
    ...
    samples: List[Dict[str, Any]] = Field(
        ..., min_items=1, max_items=10000, description="Training samples"
    )
    ...
    def validate_samples(cls, v):
        ...
        for i, sample in enumerate(v):
            ...
            if not isinstance(sample["input"], dict) or not isinstance(
                sample["output"], dict
            ):
...
class ModelTrainingRequest(BaseSchema):
    ...
    dataset_ids: List[str] = Field(
        ..., min_items=1, max_items=10, description="Training dataset IDs"
    )
    ...
    hyperparameters: Optional[Dict[str, Any]] = Field(
        default={}, description="Model-specific hyperparameters"
    )
    ...
    max_training_time_hours: int = Field(
        24, ge=1, le=168, description="Max training time in hours"
    )
...
class ModelEvaluationRequest(BaseSchema):
    ...
    test_data: List[Dict[str, Any]] = Field(
        ..., min_items=1, max_items=1000, description="Test samples"
    )
    ...
    def validate_test_data(cls, v):
        ...
        for i, sample in enumerate(v):
            if "input" not in sample or "output" not in sample:
                raise ValueError(
                    f'Test sample {i} must contain "input" and "output" fields'
                )
        return v
...
class PredictionRequest(BaseSchema):
    ...
    include_explanation: bool = Field(
        True, description="Include explanation/reasoning?"
    )
...
class TrainingDataExportRequest(BaseSchema):
    ...
    dataset_ids: List[str] = Field(
        ..., min_items=1, description="Dataset IDs to export"
    )
    ...
    date_range_start: Optional[datetime] = Field(
        None, description="Export data from this date"
    )
    date_range_end: Optional[datetime] = Field(
        None, description="Export data until this date"
    )
...
class BatchPredictionRequest(BaseSchema):
    ...
    model_type: ModelType = Field(..., description="Type of model to use")
    input_samples: List[Dict[str, Any]] = Field(
        ..., min_items=1, max_items=1000, description="Input samples"
    )
