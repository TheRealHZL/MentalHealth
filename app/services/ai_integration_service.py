...
from app.models import DreamEntry, MoodEntry, TherapyNote
...
class AIIntegrationService:
    ...
    async def analyze_mood_entry(self, mood_entry: MoodEntry) -> Dict[str, Any]:
        ...
        try:
            ...
            insights_prompt = self._build_insights_prompt(
                mood_entry, emotion_result, sentiment_result
            )
    ...
    async def analyze_dream_entry(self, dream_entry: DreamEntry) -> Dict[str, Any]:
        ...
        try:
            ...
            dream_emotion = await self.ai_engine.predict_emotion(
                text=dream_text, context={"type": "dream_content"}
            )
            ...
            return {
                ...
                "psychological_insights": await self._generate_dream_insights(
                    dream_entry, dream_emotion
                ),
                "analysis_timestamp": datetime.utcnow().isoformat(),
            }
    ...
    async def analyze_therapy_note(self, therapy_note: TherapyNote) -> Dict[str, Any]:
        ...
        try:
            ...
            note_emotion = await self.ai_engine.predict_emotion(
                text=note_text, context={"type": "therapy_note"}
            )
            ...
            return {
                ...
                "suggested_next_steps": await self._suggest_therapy_next_steps(
                    therapy_note
                ),
                "analysis_timestamp": datetime.utcnow().isoformat(),
            }
            ...
            return {
                "ai_generated": False,
                "error": "AI-Therapieanalyse nicht verfügbar",
            }

    async def generate_personalized_insights(
        self, user_id: str, recent_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            mood_forecast = await self.ai_engine.predict_mood(
                text=insights_prompt, metadata=recent_data.get("metadata", {})
            )
            ...
            return {
                "ai_generated": False,
                "error": "Personalisierte Insights nicht verfügbar",
            }
    ...
    def _build_insights_prompt(
        self, mood_entry: MoodEntry, emotion_result: Dict, sentiment_result: Dict
    ) -> str:
...
Identifiziere Muster, Fortschritte und Bereiche für Verbesserung. Gib konkrete, umsetzbare Empfehlungen für die mentale Gesundheit."""
    ...
    def _get_basic_analysis(self, mood_entry: MoodEntry) -> Dict[str, Any]:
        ...
        return {
            "mood_category": (
                ...
                else "hoch" if mood_entry.mood_score >= 7 else "moderat"
            ),
            "stress_assessment": (
                "hoch"
                if mood_entry.stress_level >= 7
                else "niedrig" if mood_entry.stress_level <= 3 else "moderat"
            ),
            "basic_recommendation": (
                "Achte heute besonders auf Selbstfürsorge."
                if mood_entry.mood_score <= 4
                else "Nutze diese positive Energie!"
            ),
        }

    async def _generate_dream_insights(
        self, dream_entry: DreamEntry, emotion_result: Dict
    ) -> List[str]:
    ...
    async def _analyze_goal_progress(self, therapy_note: TherapyNote) -> Dict[str, Any]:
        ...
        response = await self.ai_engine.generate_chat_response(
            user_message=progress_prompt, user_context={"mode": "goal_assessment"}
        )
    ...
    async def _suggest_therapy_next_steps(self, therapy_note: TherapyNote) -> List[str]:
        ...
        response = await self.ai_engine.generate_chat_response(
            user_message=prompt, user_context={"mode": "therapy_planning"}
        )

        # Parse response in Liste (vereinfacht)
        return [
            step.strip() for step in response["response"].split("\n") if step.strip()
        ][:3]
