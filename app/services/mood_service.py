...
from app.schemas.ai import MoodEntryCreate, MoodEntryUpdate, PaginationParams
...
class MoodService:
    ...
    async def create_mood_entry(
        self, user_id: str, mood_data: MoodEntryCreate
    ) -> MoodEntry:
    ...
    async def create_quick_mood_entry(
        self, user_id: str, mood_score: int, note: Optional[str] = None
    ) -> MoodEntry:
    ...
    async def get_mood_entry_by_date(
        self, user_id: str, entry_date: date, entry_time: time
    ) -> Optional[MoodEntry]:
    ...
    async def get_mood_entries_paginated(
        ...
        count_query = select(func.count(MoodEntry.id)).where(
            MoodEntry.user_id == uuid.UUID(user_id)
        )
    ...
    async def update_mood_entry(
        self, entry_id: str, user_id: str, update_data: MoodEntryUpdate
    ) -> MoodEntry:
        """Update mood entry"""
