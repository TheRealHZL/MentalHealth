...
class Settings(BaseSettings):
    ...
    # Feature Flags
    THERAPIST_VERIFICATION_REQUIRED: bool = Field(default=True, env="THERAPIST_VERIFICATION_REQUIRED")
    EMAIL_VERIFICATION_REQUIRED: bool = Field(default=False, env="EMAIL_VERIFICATION_REQUIRED")
    ...
    DEFAULT_SHARE_KEY_EXPIRY_DAYS: int = Field(default=90, env="DEFAULT_SHARE_KEY_EXPIRY_DAYS")
...
def validate_configuration(settings: Settings):
    ...
    if settings.EMAIL_VERIFICATION_REQUIRED:
        if not all([settings.SMTP_HOST, settings.SMTP_USERNAME, settings.SMTP_PASSWORD]):
            errors.append("SMTP configuration required when EMAIL_VERIFICATION_REQUIRED is True")
