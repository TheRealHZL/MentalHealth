...
class ChatSession(Base):
    ...
    session_type = Column(
        String(30), nullable=False, default="general_support"
    )  # general, crisis, therapy, mood
    session_title = Column(String(200), nullable=True)
    session_goals = Column(ARRAY(String), nullable=True)
    therapeutic_approach = Column(
        String(50), nullable=True
    )  # person_centered, cbt, dbt, etc.
    ...
    mood_progression = Column(
        JSON, nullable=True
    )  # Array of mood changes throughout session
    context_data = Column(
        JSON, nullable=True
    )  # Additional context from previous entries
    ...
    crisis_interventions = Column(
        JSON, nullable=True
    )  # Array of interventions triggered
    ...
    analyses = relationship(
        "ChatAnalysis", back_populates="session", cascade="all, delete-orphan"
    )
...
class ChatMessage(Base):
    ...
    message_type = Column(
        String(30), nullable=False, default="chat"
    )  # chat, greeting, response, system, crisis
    ...
    user_intent = Column(
        String(50), nullable=True
    )  # question, vent, seek_advice, crisis, etc.
    ...
    therapeutic_intent = Column(
        String(50), nullable=True
    )  # support, clarify, validate, challenge, etc.
...
class ChatAnalysis(Base):
    ...
    )  # sentiment, topic, progress, crisis, effectiveness
    analysis_scope = Column(
        String(20), nullable=False, default="full_session"
    )  # full_session, recent, custom_range
...
class ChatTemplate(Base):
    ...
    template_type = Column(
        String(30), nullable=False
    )  # greeting, crisis, closure, check_in
...
class ConversationFlow(Base):
    ...
    flow_type = Column(
        String(30), nullable=False
    )  # guided_therapy, crisis_intervention, check_in
    ...
    therapeutic_approaches = Column(
        ARRAY(String), nullable=True
    )  # Compatible approaches
    target_conditions = Column(
        ARRAY(String), nullable=True
