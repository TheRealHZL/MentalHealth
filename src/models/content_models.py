...
class MoodEntry(Base):
    ...
    def calculate_wellness_score(self) -> float:
        ...
        )  # Invert stress (higher stress = lower wellness)
...
class DreamEntry(Base):
    ...
    lucidity_trigger = Column(
        String(200), nullable=True
    )  # What made you realize it was a dream
    ...
    recurring_elements = Column(
        ARRAY(String), nullable=True
    )  # Elements from other dreams
...
class TherapyNote(Base):
    ...
    note_type = Column(
        String(30), nullable=False, default=TherapyNoteType.SELF_REFLECTION.value
    )
    ...
    def to_dict(
        self, include_ai_data: bool = True, include_private: bool = False
    ) -> dict:
        """Convert to dictionary"""
        base_dict = {
            "id": str(self.id),
            ...
            "content": (
                self.content
                if include_private or not self.is_private
            ...
            "key_insights": (
                self.key_insights if include_private or not self.is_private else None
            ),
...
Index(
    "idx_therapy_notes_shareable",
    TherapyNote.share_with_therapist,
    TherapyNote.note_date,
)
