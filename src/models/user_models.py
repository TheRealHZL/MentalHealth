...
class User(Base):
    ...
    dream_entries = relationship(
        "DreamEntry", back_populates="user", cascade="all, delete-orphan"
    )
    therapy_notes = relationship(
        "TherapyNote", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    therapist_share_keys = relationship(
        "ShareKey", foreign_keys="ShareKey.therapist_id", back_populates="therapist"
    )
    ...
    training_jobs = relationship(
        "TrainingJob", back_populates="starter", foreign_keys="TrainingJob.started_by"
    )
...
class LoginAttempt(Base):
    ...
    )  # Nullable for failed email attempts
...
class UserNotification(Base):
    ...
    )  # "welcome", "share_key_accepted", "therapist_verified"
...
class UserActivityLog(Base):
    ...
    activity_type = Column(
        String(50), nullable=False
    )  # "login", "mood_entry", "share_key_created"
...
Index(
    "idx_notifications_user_unread", UserNotification.user_id, UserNotification.is_read
)
Index(
    "idx_activity_logs_user_type",
