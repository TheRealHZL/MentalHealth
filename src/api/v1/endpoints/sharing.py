...
from src.schemas.ai import (
    ShareKeyCreate, ShareKeyResponse, TherapistAccessRequest,
    PatientOverview, SuccessResponse, PaginationParams, PaginatedResponse
)
...
async def create_share_key(
    ...
    try:
        ...
        if not therapist:
            ...
            logger.info(f"Share key created for unregistered therapist: {share_data.therapist_email}")
        ...
        logger.info(f"Share key created by patient {user_id} for therapist {share_data.therapist_email}")
...
async def accept_share_key(
    ...
    try:
        ...
        logger.info(f"Share key accepted by therapist {user_id}: {access_request.share_key[:8]}...")
...
async def revoke_share_key(
    ...
    try:
        ...
        await sharing_service.notify_therapist_of_revocation(
            share_key_id=share_key_id,
            patient_name=revoked_key.patient.first_name
        )
...
async def get_access_logs(
    ...
    try:
        ...
        if not share_key:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Share-Key nicht gefunden"
            )
        ...
        return {
            ...
            "summary": {
                ...
                "therapist_name": f"Dr. {share_key.therapist.last_name}" if share_key.therapist else "Unbekannt"
            }
        }
...
async def emergency_revoke_all(
    ...
    try:
        ...
        logger.warning(f"EMERGENCY: Patient {user_id} revoked ALL share keys ({revoked_count} keys)")
