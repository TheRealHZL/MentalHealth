...
from src.schemas.ai import (
    TrainingDataUpload, TrainingDatasetCreate, TrainingDatasetResponse,
    ModelTrainingRequest, ModelTrainingResponse, ModelVersionResponse,
    TrainingJobStatus, TrainingMetrics, ModelEvaluationRequest,
...
@router.post("/datasets", response_model=TrainingDatasetResponse, status_code=status.HTTP_201_CREATED)
async def create_training_dataset(
    ...
    **Admin/Therapist Only**: Only verified users can create training datasets
    ...
    try:
        ...
        logger.info(f"Training dataset created: {new_dataset.id} by user {current_user_id}")
...
async def list_training_datasets(
    ...
    try:
        ...
        for dataset in datasets:
            items.append(TrainingDatasetResponse(
                id=dataset.id,
                name=dataset.name,
                description=dataset.description,
                dataset_type=dataset.dataset_type,
...
async def get_training_status(
    ...
    try:
        ...
        return TrainingJobStatus(
            ...
            logs=training_job.logs[-50:] if training_job.logs else []  # Last 50 log entries
        )
...
async def evaluate_model_performance(
    ...
    try:
        ...
        logger.info(f"Model evaluation completed: {model_id} - Accuracy: {accuracy:.2%}")
...
def compare_predictions(predicted: Dict[str, Any], actual: Dict[str, Any]) -> bool:
    ...
    return predicted.get("mood_category") == actual.get("mood_category") or \
           len(predicted.get("emotional_themes", [])) > 0
