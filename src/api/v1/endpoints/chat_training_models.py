...
from src.schemas.chat import (ChatModelTrainingRequest,
...
@router.post(
    "/train",
    response_model=ChatModelTrainingResponse,
...
async def train_chat_model(
    ...
    **Training Configuration:**
    ...
    try:
        ...
        estimated_hours = (
            total_samples * training_request.training_config.get("epochs", 3)
        ) / 1000  # Rough estimate
...
async def stop_chat_training(
    ...
    try:
        ...
        training_job.logs.append(
            f"Training stopped by admin at {datetime.utcnow().isoformat()}"
        )
...
async def list_chat_training_jobs(
    ...
    try:
        ...
        jobs = (
            await query.order_by(TrainingJob.started_at.desc())
            .offset(offset)
            .limit(page_size)
            .all()
        )
...
async def execute_chat_model_training(job_id: str, training_config, db):
    ...
    try:
        ...
        job.current_step = 1
        job.total_steps = (
            training_config.training_config.get("epochs", 3) * 10
        )  # Steps per epoch
        await db.commit()
        ...
        for epoch in range(1, epochs + 1):
            ...
            for step in range(1, 11):  # 10 steps per epoch
                ...
                training_metrics["total_loss"] = (
                    ...
                    + training_metrics["therapeutic_loss"]
                    * training_config.therapeutic_weight
                )
                ...
                if not job.logs:
                    job.logs = []
                job.logs.append(
                    f"Epoch {epoch}/{epochs} Step {step}/10: Training chat model... Loss: {training_metrics['total_loss']:.4f}"
                )
