...
from src.schemas.ai import PaginatedResponse, SuccessResponse
...
@router.post(
    "/datasets",
    response_model=ChatTrainingDataResponse,
    ...
    try:
        ...
        chat_dataset = TrainingDataset(
            ...
            metadata={
                ...
                "average_ratings": {
                    "safety": sum(p.safety_rating for p in dataset.training_pairs)
                    / len(dataset.training_pairs),
                    "empathy": sum(p.empathy_rating for p in dataset.training_pairs)
                    / len(dataset.training_pairs),
                    "therapeutic_quality": sum(
                        p.therapeutic_quality for p in dataset.training_pairs
                    )
                    / len(dataset.training_pairs),
                },
            },
        ...
        high_quality_pairs = sum(
            ...
            if (pair.safety_rating + pair.empathy_rating + pair.therapeutic_quality) / 3
            >= 4.0
        )
        ...
        return ChatTrainingDataResponse(
            ...
            average_safety_rating=sum(p.safety_rating for p in dataset.training_pairs)
            / len(dataset.training_pairs),
            average_empathy_rating=sum(p.empathy_rating for p in dataset.training_pairs)
            / len(dataset.training_pairs),
            average_therapeutic_quality=sum(
                p.therapeutic_quality for p in dataset.training_pairs
            )
            / len(dataset.training_pairs),
            therapeutic_focus=dataset.therapeutic_focus,
...
async def list_chat_training_datasets(
    ...
    try:
        ...
        query = db.query(TrainingDataset).filter(
            TrainingDataset.dataset_type == "chat_conversation"
        )

        # Filter by language if specified
        if language:
            query = query.filter(
                TrainingDataset.metadata["language"].astext == language
            )
...
async def process_chat_training_data(
    dataset_id: str, training_pairs: List, uploaded_by: str, db
):
    ...
    try:
        ...
        logger.info(
            f"Chat training data processed successfully for dataset {dataset_id}"
        )
