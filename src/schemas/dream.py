...
class DreamEntryCreate(BaseModel):
    ...
    dream_date: date = Field(
        default_factory=lambda: date.today(), description="Dream date"
    )
    ...
    description: str = Field(
        ..., min_length=10, max_length=5000, description="Dream description"
    )

    vividness: Optional[int] = Field(
        None, ge=1, le=10, description="Dream vividness (1-10)"
    )
    emotional_intensity: Optional[int] = Field(
        None, ge=1, le=10, description="Emotional intensity (1-10)"
    )
    mood_during_dream: Optional[List[int]] = None
    mood_after_waking: int = Field(
        ..., ge=1, le=10, description="Mood after waking (1-10)"
    )
    ...
    sleep_duration: Optional[float] = Field(
        None, ge=0, le=24, description="Hours slept"
    )

    became_lucid: bool = Field(False, description="Did you become lucid?")
    lucidity_trigger: Optional[str] = Field(
        None, max_length=200, description="What made you realize it was a dream"
    )
    lucid_actions: Optional[List[str]] = None
    lucid_control_level: Optional[int] = Field(
        None, ge=1, le=10, description="Level of control when lucid"
    )

    personal_interpretation: Optional[str] = Field(
        None, max_length=2000, description="Your interpretation"
    )
    life_connection: Optional[str] = Field(
        None, max_length=1000, description="Connection to real life"
    )
    recurring_elements: Optional[List[str]] = None

    dream_recall_clarity: Optional[int] = Field(
        None, ge=1, le=10, description="How clearly remembered"
    )
    entry_delay_hours: Optional[int] = Field(
        None, ge=0, le=168, description="Hours between dream and entry"
    )
    ...
    @field_validator(
        "description", "personal_interpretation", "life_connection", mode="before"
    )
    @classmethod
