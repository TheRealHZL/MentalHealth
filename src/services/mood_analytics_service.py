...
class MoodAnalyticsService:
    ...
    async def analyze_mood_entry(self, mood_entry: MoodEntry) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "sleep_impact": self._analyze_sleep_impact(mood_entry.sleep_quality, mood_entry.sleep_hours),
            "activity_benefits": self._analyze_activities(mood_entry.activities, mood_entry.exercise_minutes),
    ...
    async def get_mood_correlations(self, user_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        mood_sleep_corr = self._calculate_correlation(
            ...
            [e.sleep_hours for e in entries]
        )
        
        mood_exercise_corr = self._calculate_correlation(
            [e.mood_score for e in entries],
            [e.exercise_minutes for e in entries]
        )
        ...
        return {
            ...
            "exercise_correlation": {
                ...
                "interpretation": self._interpret_correlation(mood_exercise_corr, "exercise")
            },
            "stress_correlation": {
                "score": mood_stress_corr,
                "interpretation": self._interpret_correlation(mood_stress_corr, "stress")
            },
            "insights": self._generate_correlation_insights(mood_sleep_corr, mood_exercise_corr, mood_stress_corr)
        }
    ...
    async def generate_mood_insights(self, user_id: str) -> List[str]:
        ...
        if len(recent_entries) >= 3:
            ...
            if avg_mood >= 7:
                ...
                insights.append("💙 Du durchlebst eine schwierige Phase. Das ist normal und geht vorbei.")
            
            # Check consistency
            mood_variance = self._calculate_variance([e.mood_score for e in recent_entries])
            if mood_variance < 1.5:
                insights.append("🎯 Deine Stimmung ist sehr stabil - das zeigt emotionale Balance.")
        ...
        if sleep_entries:
            ...
            if avg_sleep < 6:
                insights.append("😴 Mehr Schlaf könnte deine Stimmung deutlich verbessern.")
            elif avg_sleep > 9:
                insights.append("💤 Du schläfst viel - achte auf die Schlafqualität, nicht nur die Dauer.")
        ...
        if len(exercise_entries) >= len(recent_entries) * 0.7:
            ...
            insights.append("🚶 Schon 10 Minuten Bewegung täglich können deine Stimmung heben.")
    ...
    def _assess_energy(self, energy_level: int) -> Dict[str, Any]:
        """Assess energy level"""
        if energy_level <= 3:
            return {"category": "low", "message": "Niedrige Energie - mehr Erholung nötig. 🔋"}
        elif energy_level <= 6:
            return {"category": "moderate", "message": "Moderate Energie - ausgewogen. ⚡"}
        else:
            return {"category": "high", "message": "Hohe Energie - nutze sie! 🚀"}
    
    def _analyze_sleep_impact(self, sleep_quality: int, sleep_hours: float) -> Dict[str, Any]:
        """Analyze sleep impact on mood"""
        
        quality_assessment = "gut" if sleep_quality >= 7 else "schlecht" if sleep_quality <= 4 else "okay"
        duration_assessment = "zu wenig" if sleep_hours < 6 else "zu viel" if sleep_hours > 9 else "angemessen"
        
        return {
            "quality": quality_assessment,
            "duration": duration_assessment,
            "recommendation": self._get_sleep_recommendation(sleep_quality, sleep_hours)
        }
    
    def _analyze_activities(self, activities: List[str], exercise_minutes: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": self._get_activity_recommendation(activity_count, exercise_minutes)
        }
    ...
    def _check_risk_indicators(self, mood_entry: MoodEntry) -> List[str]:
        ...
        if mood_entry.symptoms and any("suizid" in symptom.lower() for symptom in mood_entry.symptoms):
    ...
    def _get_activity_recommendation(self, activity_count: int, exercise_minutes: int) -> str:
