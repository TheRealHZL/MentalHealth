...
class DreamService:
    ...
    async def get_dream_entries_paginated(
        ...
        count_query = select(func.count(DreamEntry.id)).where(DreamEntry.user_id == uuid.UUID(user_id))
        ...
        if dream_type:
            try:
                ...
                count_query = count_query.where(DreamEntry.dream_type == dream_type_enum)
            except ValueError:
    ...
    async def update_ai_analysis(
        ...
        if dream_entry:
            ...
            dream_entry.emotional_insights = ai_analysis.get("psychological_insights", [])
    ...
    async def get_dream_statistics(self, user_id: str, days: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "nightmares": sum(1 for entry in entries if entry.dream_type == DreamType.NIGHTMARE),
    ...
    async def analyze_dream_patterns(self, user_id: str, days: int) -> Dict[str, Any]:
        ...
        if len(entries) < 3:
            return {"total_dreams": len(entries), "message": "Mehr Träume für Muster-Analyse benötigt"}
    ...
    async def get_common_dream_elements(self, user_id: str, limit: int = 30) -> Dict[str, Any]:
    ...
    async def analyze_dream_quality_trends(self, user_id: str) -> Dict[str, Any]:
        ...
        # Get last 30 dreams
        result = await self.db.execute(
            select(DreamEntry.mood_after_waking, DreamEntry.sleep_quality, DreamEntry.dream_date).where(
                DreamEntry.user_id == uuid.UUID(user_id)
            ).order_by(desc(DreamEntry.dream_date)).limit(30)
        )
        ...
        if mid_point > 0:
            ...
            mood_trend = "improving" if late_mood > early_mood + 0.5 else "declining" if late_mood < early_mood - 0.5 else "stable"
            
            early_sleep = sum(sleep_scores[:mid_point]) / mid_point
            late_sleep = sum(sleep_scores[mid_point:]) / (len(sleep_scores) - mid_point)
            sleep_trend = "improving" if late_sleep > early_sleep + 0.5 else "declining" if late_sleep < early_sleep - 0.5 else "stable"
        else:
            mood_trend = "stable"
            sleep_trend = "stable"
    ...
    def _get_mood_range_filter(self, mood_range: str):
        ...
        if mood_range == "low":
            ...
            return and_(DreamEntry.mood_after_waking > 4, DreamEntry.mood_after_waking <= 7)
