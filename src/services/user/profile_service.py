...
from src.models import User, UserRole, MoodEntry, DreamEntry, TherapyNote, ShareKey, ShareKeyAccessLog
...
class ProfileService:
    ...
    async def _get_patient_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "usage_insights": {
                "avg_entries_per_week": round((mood_count / max(days_registered / 7, 1)), 1),
                "most_used_feature": self._determine_most_used_feature(mood_count, dream_count, therapy_count),
                "consistency_score": self._calculate_consistency_score(recent_mood_count, days_registered)
            },
            "milestones": self._get_patient_milestones(mood_count, dream_count, therapy_count, days_registered)
        }
    ...
    async def _get_therapist_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        auth_service = AuthService(self.db)
        user = await auth_service.get_user_by_id(user_id)
        days_since_verification = (datetime.utcnow() - user.created_at).days if user.is_verified else 0
        ...
        return {
            ...
            "recent_activity": {
                ...
                "avg_accesses_per_patient": round(total_accesses / max(active_patients, 1), 1)
            },
            "professional_info": {
            ...
            "practice_metrics": {
                "patient_engagement": "high" if recent_accesses > 5 else "medium" if recent_accesses > 0 else "low",
                "data_utilization": round((total_accesses / max(active_patients * 30, 1)) * 100, 1)  # % of available data accessed
            }
        }
    
    def _determine_most_used_feature(self, mood_count: int, dream_count: int, therapy_count: int) -> str:
    ...
    def _calculate_consistency_score(self, recent_entries: int, days_registered: int) -> int:
    ...
    def _get_patient_milestones(self, mood_count: int, dream_count: int, therapy_count: int, days_registered: int) -> List[str]:
    ...
    async def get_profile_completion_status(self, user_id: str) -> Dict[str, Any]:
        ...
        if user.role == UserRole.PATIENT:
            patient_items = [
                ("first_entry", "Erster Mood-Eintrag", await self._has_mood_entries(user_id)),
                ("profile_picture", "Profilbild hochgeladen", bool(user.profile_picture_url)),
            ]
            ...
            therapist_items = [
                ...
                ("bio_complete", "Bio ausgefüllt", bool(user.bio and len(user.bio) > 50)),
                ("specializations", "Spezialisierungen angegeben", bool(user.specializations)),
        ...
        completion_percentage = round((completed_count / total_items) * 100) if total_items > 0 else 0
        ...
        return {
            ...
            "next_suggestions": self._get_completion_suggestions(completion_items, user.role)
        }
    
    def _get_completion_suggestions(self, items: List[Dict], role: UserRole) -> List[str]:
