...
from src.models import (
    User, UserRole, MoodEntry, DreamEntry, TherapyNote, ShareKey,
    ShareKeyAccessLog, LoginAttempt
)
...
class DataService:
    ...
    async def _export_sharing_data(self, user_id: str, role: UserRole) -> Dict[str, Any]:
    ...
    async def get_platform_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "engagement_metrics": {
                "avg_entries_per_user": round((total_mood + total_dreams + total_therapy) / max(total_users, 1), 1),
                "patient_therapist_ratio": round(patients_count / max(therapists_count, 1), 1),
                "sharing_adoption_rate": round((active_shares / max(patients_count, 1)) * 100, 1),
                "recent_activity_30d": {
            ...
            "platform_health": {
                ...
                "content_creation": "active" if recent_mood > 100 else "moderate" if recent_mood > 20 else "low",
                "therapeutic_engagement": "high" if active_shares > patients_count * 0.1 else "moderate"
            }
        }
