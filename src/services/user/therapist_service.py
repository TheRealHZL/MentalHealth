...
from src.models import User, UserRole, ShareKey, ShareKeyAccessLog
...
class TherapistService:
    ...
    async def search_verified_therapists(
        ...
        if specialization:
            query = query.where(
                or_(
                    func.array_to_string(User.specializations, ',').ilike(f'%{specialization}%'),
        ...
        for therapist in therapists:
            formatted_therapists.append({
                ...
                "practice_location": self._extract_city_from_address(therapist.practice_address),
                "email": therapist.email,  # For share key creation
                "verified_since": therapist.created_at.strftime("%Y") if therapist.is_verified else None,
                "patient_count": await self._get_therapist_patient_count(str(therapist.id)),
                "experience_level": self._determine_experience_level(therapist.created_at),
                "availability_status": await self._get_availability_status(str(therapist.id))
            })
        
        return formatted_therapists
    
    async def get_therapist_profile(self, therapist_id: str, requesting_user_role: str) -> Dict[str, Any]:
        ...
        if requesting_user_role == "patient":
            profile.update({
                ...
                "practice_city": self._extract_city_from_address(therapist.practice_address),
                ...
                "experience_years": self._calculate_experience_years(therapist.created_at),
                "contact_info": {
        ...
        # Full info for the therapist themselves
            ...
            profile.update({
                ...
                "verification_status": "verified" if therapist.is_verified else "pending",
                "professional_stats": await self._get_professional_statistics(therapist_id)
            })
    ...
    async def verify_therapist(self, therapist_id: str, admin_notes: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def reject_therapist_verification(
        ...
        logger.warning(f"Therapist verification rejected: {therapist.email} - {rejection_reason}")
    ...
    async def get_therapist_patients_overview(self, therapist_id: str) -> Dict[str, Any]:
        ...
        if patients_overview:
            avg_mood = sum(p.get("avg_mood_30d", 0) for p in patients_overview if p.get("avg_mood_30d")) / len([p for p in patients_overview if p.get("avg_mood_30d")])
            most_common_challenges = self._aggregate_patient_challenges(patients_overview)
        else:
            avg_mood = 0
        ...
        return {
            ...
            "aggregate_insights": {
                ...
                "engagement_distribution": self._calculate_engagement_distribution(patients_overview)
            },
            "clinical_notes": {
    ...
    async def get_patient_clinical_summary(
        ...
        clinical_summary = {
            ...
            "clinical_observations": self._generate_clinical_observations(mood_data, therapy_data),
            "recommended_focus_areas": self._suggest_focus_areas(mood_data, therapy_data),
            "data_quality": {
                "mood_data_points": len(mood_data.get("entries", [])),
                "therapy_entries": len(therapy_data.get("entries", [])),
                "data_completeness": self._assess_data_completeness(mood_data, therapy_data, days)
            }
        }
    ...
    def _sanitize_bio_for_public(self, bio: Optional[str]) -> Optional[str]:
        ...
        import re
        sanitized = re.sub(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', '[Email entfernt]', sanitized)
        sanitized = re.sub(r'\b\d{2,4}[-.\s]?\d{3,4}[-.\s]?\d{3,4}\b', '[Telefonnummer entfernt]', sanitized)
    ...
    async def _get_professional_statistics(self, therapist_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_accesses_per_patient": round(total_accesses / max(patient_count, 1), 1),
            "engagement_level": "high" if recent_activity > 10 else "medium" if recent_activity > 3 else "low"
        }
    
    async def _get_patient_summary_for_therapist(self, share_key: ShareKey) -> Dict[str, Any]:
        ...
        patient_stats = await sharing_service._get_patient_summary_stats(str(share_key.patient_id))
    ...
    def _generate_clinical_observations(self, mood_data: Dict, therapy_data: Dict) -> List[str]:
        ...
        if mood_summary.get("avg_mood"):
            ...
            if avg_mood < 4:
                observations.append("Durchschnittliche Stimmung liegt im niedrigen Bereich")
            elif avg_mood > 7:
    ...
    def _assess_data_completeness(self, mood_data: Dict, therapy_data: Dict, days: int) -> str:
    ...
    def _calculate_engagement_distribution(self, patients: List[Dict]) -> Dict[str, int]:
        """Calculate patient engagement distribution"""
        
        high_engagement = sum(1 for p in patients if p.get("mood_entries_30d", 0) > 15)
        medium_engagement = sum(1 for p in patients if 5 <= p.get("mood_entries_30d", 0) <= 15)
