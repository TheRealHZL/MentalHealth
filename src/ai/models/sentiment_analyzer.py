...
class EmotionalContextEncoder(nn.Module):
    ...
    def __init__(self, vocab_size: int, embed_dim: int):
        ...
        self.polarity_embeddings = nn.Embedding(3, embed_dim)  # negative, neutral, positive
    ...
    def forward(
        ...
        combined_embeds = torch.cat([
            word_embeds, polarity_embeds, intensity_embeds
        ], dim=-1)
...
class SentimentAnalyzer(nn.Module):
    ...
    def forward(
        ...
        embeddings_transposed = embeddings.transpose(1, 2)  # [batch_size, embedding_dim, seq_len]
    ...
    def analyze_emotional_aspects(
        ...
        with torch.no_grad():
            ...
            polarity_score = probabilities[:, 2] - probabilities[:, 0]  # positive - negative
            ...
            for i in range(input_ids.size(0)):
                result = {
                    'basic_sentiment': sentiment_prediction if isinstance(sentiment_prediction, dict) else sentiment_prediction[i],
                    'polarity_score': polarity_score[i].item(),  # -1 (negative) to 1 (positive)
                    'emotional_stability': stability[i].item(),  # 0 (unstable) to 1 (stable)
