    ...
    def __init__(
        ...
        if is_causal:
            self.register_buffer(
                "causal_mask", torch.tril(torch.ones(1024, 1024)).view(1, 1, 1024, 1024)
            )
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        residual = x
        x = self.norm1(x)
        attn_out, present_key_value = self.self_attention(
...
class EmpathyEncoder(nn.Module):
    ...
    def forward(
        ...
        if emotion_ids is not None:
            emotion_embeds = self.emotion_embeddings(
                emotion_ids
        ...
        if mood_scores is not None:
            ...
            mood_embeds = (
                mood_normalized.unsqueeze(1).unsqueeze(2).expand(-1, seq_len, embed_dim)
            )
...
class ChatGenerator(nn.Module):
    ...
    def forward(
        ...
        if return_safety_scores:
            ...
            outputs.update(
                {"safety_scores": safety_scores, "empathy_scores": empathy_scores}
            )
    ...
    def compute_loss(
        ...
        outputs = self.forward(
            ...
            emotion_context=emotion_context,
            mood_context=mood_context,
            return_safety_scores=(
                safety_labels is not None or empathy_labels is not None
            ),
        )
        ...
        if empathy_labels is not None and "empathy_scores" in outputs:
            empathy_loss = F.mse_loss(
                outputs["empathy_scores"].squeeze(-1), empathy_labels.float()
            )
        ...
        loss_dict.update({"total_loss": total_loss, "perplexity": perplexity})
