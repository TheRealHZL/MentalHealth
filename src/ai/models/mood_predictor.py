...
class MoodPredictor(nn.Module):
    ...
    def __init__(
        ...
        self.lstm_layers.append(BiLSTMLayer(embedding_dim, hidden_dim, dropout_rate))
    ...
    def forward(
        ...
        if (
            ...
            and self.metadata_encoder is not None
        ):
            ...
            numerical_features = metadata.get(
                "numerical_features", torch.zeros(batch_size, 5, device=device)
            )
            ...
            fused_representation = torch.cat(
                [text_representation, metadata_representation], dim=-1
            )
        else:
    ...
    def compute_loss(
        ...
        uncertainty_weighted_loss = mse_loss / (uncertainty + 1e-6) + torch.log(
            uncertainty + 1e-6
        )
