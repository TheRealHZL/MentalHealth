...
class MindBridgeTokenizer:
    ...
    def _get_word_pieces(self, word: str) -> List[str]:
        ...
        for merge_a, merge_b in self.bpe_merges:
            ...
            while i < len(word_pieces):
                if (
                    i < len(word_pieces) - 1
                    and word_pieces[i] == merge_a
                    and word_pieces[i + 1] == merge_b
                ):
        ...
        for text in texts:
            ...
            if max_length:
                if len(encoded) > max_length:
                    ...
                    # Pad
                    encoded = encoded + [self.token_to_id["<pad>"]] * (
    ...
    async def train_from_samples(self, sample_texts: Optional[List[str]] = None):
        ...
        logger.info(
            f"✅ Tokenizer training completed. Vocabulary size: {len(self.token_to_id)}"
        )
    ...
    def get_vocab_info(self) -> Dict[str, any]:
        ...
        return {
            ...
            "mental_health_terms": len(
                self.mental_health_vocab & set(self.token_to_id.keys())
            ),
