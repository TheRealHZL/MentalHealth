...
class ChatModelEvaluator:
    ...
    def evaluate_single_response(
        ...
        # Response Quality
        quality_results = self.quality_eval.evaluate_all_quality(generated_text, input_text)
    ...
    def evaluate_dataset(
        ...
        for i, sample in enumerate(test_data):
            ...
            generated_text = sample.get('generated', reference_text or f"Response to: {input_text}")
        ...
        logger.info(f"   - Empathy Score: {aggregated_results['avg_empathy_score']:.3f}")
        logger.info(f"   - Helpfulness: {aggregated_results['avg_helpfulness_score']:.3f}")
    ...
    def _aggregate_metrics(self, metrics_list: List[EvaluationMetrics]) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_semantic_similarity': np.mean([m.semantic_similarity for m in metrics_list]),
            ...
            'safe_responses_rate': np.mean([1.0 if m.safety_score > 0.7 else 0.0 for m in metrics_list]),
            
            # Empathy
            'avg_empathy_score': np.mean([m.empathy_score for m in metrics_list]),
            'avg_emotional_awareness': np.mean([m.emotional_awareness for m in metrics_list]),
            ...
            'avg_helpfulness_score': np.mean([m.helpfulness_score for m in metrics_list]),
            ...
            # Overall Score
            'overall_quality_score': np.mean([m.get_overall_score() for m in metrics_list])
        }
    ...
    def generate_report(self, results: Dict[str, Any]) -> str:
        ...
        report += f"  - Safe Responses: {results.get('safe_responses_rate', 0)*100:.1f}%\n\n"
        ...
        report += f"  - Tokens/Second: {results.get('avg_tokens_per_second', 0):.1f}\n\n"
