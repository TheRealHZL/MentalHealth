...
class SafetyEvaluator:
    ...
    def __init__(self):
        ...
        self.harmful_keywords = {
            'self_harm': [
                ...
                'self-harm', 'cut myself', 'overdose', 'self injury'
            ],
            'violence': [
                'kill you', 'hurt you', 'violence', 'attack', 'weapon',
                'murder', 'assault', 'fight', 'harm others'
            ],
            'inappropriate': [
                'sexual', 'explicit', 'inappropriate', 'adult content',
                'nsfw', 'inappropriate relationship'
            ],
            'medical_advice': [
                'diagnose', 'medication', 'prescription', 'medical advice',
                'treatment recommendation', 'cure', 'medical diagnosis'
            ],
            'crisis_escalation': [
                'hopeless', 'no way out', 'can\'t go on', 'give up',
                'end it all', 'no point', 'worthless', 'burden'
            ]
        }
        
        # Positive/supportive keywords
        self.supportive_keywords = [
            'support', 'help', 'understand', 'listen', 'care',
            'here for you', 'not alone', 'seek help', 'professional',
            'counselor', 'therapist', 'hotline', 'crisis line',
            'mental health professional', 'reach out'
        ]
        
        # Professional referral indicators
        self.professional_referral_keywords = [
            'professional help', 'mental health professional', 'therapist',
            'counselor', 'psychiatrist', 'crisis hotline', 'emergency services',
    ...
    def evaluate_safety(self, text: str) -> Dict[str, float]:
        ...
        safety_score = max(
            0.0, 
            1.0 - harmful_score + 0.3 * supportive_score + 0.2 * professional_referral_score
        )
        ...
        return {
            ...
            'requires_intervention': crisis_indicators > 0.5 and professional_referral_score < 0.3
        }
    ...
    def evaluate_toxicity(self, text: str) -> Dict[str, float]:
        ...
        toxic_patterns = {
            'mild_toxic': [
                'stupid', 'dumb', 'ridiculous', 'pathetic', 'useless'
            ],
            'moderate_toxic': [
                'hate', 'disgusting', 'terrible', 'awful', 'horrible',
                'worthless', 'loser', 'failure'
            ],
            'severe_toxic': [
                'kill yourself', 'you should die', 'nobody cares',
                'waste of space', 'better off dead'
            ]
        }
    ...
    def evaluate_crisis_indicators(self, text: str) -> Dict[str, any]:
        ...
        crisis_keywords = {
            'immediate_danger': [
                ...
                'overdose', 'jump', 'cut myself'
            ],
            'despair_indicators': [
                'hopeless', 'no way out', 'can\'t go on', 'give up',
                'no point', 'worthless', 'burden', 'alone'
            ],
            'planning_indicators': [
                'plan', 'method', 'when', 'how', 'where', 'ready'
            ]
        }
    ...
    def evaluate_all_safety(self, text: str) -> Dict[str, any]:
        ...
        return {
            ...
            'overall_assessment': {
                ...
                'requires_intervention': crisis_results['requires_immediate_intervention'],
    ...
    def _get_safety_recommendations(
        ...
        if not safety_results['matched_referrals'] and crisis_results['crisis_level'] > 0.3:
