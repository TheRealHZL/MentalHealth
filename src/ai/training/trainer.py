...
from .utils import (AverageMeter, EarlyStopping, count_parameters, format_time,
                    get_lr, save_checkpoint, set_seed)
...
class ModelTrainer:
            ...
            for batch in tqdm(self.val_loader, desc="Validation"):
                ...
                losses.update(
                    ...
                    (
                        ...
                        else batch["mood_sequence"].size(0)
                    ),
                )
    ...
    def _compute_loss(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.config.model_type in ["sentiment_analyzer", "emotion_classifier"]:
            ...
            # Regression
            outputs = self.model(
                mood_sequence=batch["mood_sequence"], metadata=batch.get("metadata")
            )
